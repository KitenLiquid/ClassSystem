...
public class AddServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(service.add(stu1)!=null&&service.add1(stu1)!=null&&service.add2(stu1)!=null)
		{
			...
			request.getSession().setAttribute("stu1",stu1);  //灏嗘煡璇㈢粨鏋滃啓鍏ession鍩熷璞�
        	RequestDispatcher rd=request.getRequestDispatcher("SListServlet2");
			rd.forward(request, response);

		}
		else
		{
			response.getWriter().write("<h3>璇ュ鍙峰凡瀛樺湪锛岃閲嶆柊杈撳叆銆�</h3>");
